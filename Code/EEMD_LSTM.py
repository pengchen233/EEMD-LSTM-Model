...
def LSTM_Model(X_train, Y_train,i):
    ...
    model = Sequential()
    model.add(LSTM(50,activation='relu', input_shape=(X_train.shape[1], X_train.shape[2])))  # 已经确定10步长
    '''
    ...
    model.fit(X_train, Y_train, epochs=20, batch_size=16, validation_split=0.1, verbose=2, shuffle=True)
    return (model)
...
if __name__ == '__main__':
    ...
    #    model.save('EEMD_LSTM_5050/EEMD-LSTM-imf' + str(i) + '-50.h5')
    #    prediction_Y = model.predict(X2_test)
    #    imfs_prediction.append(prediction_Y)
    #    i+=1;
